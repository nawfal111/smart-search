...
//   AI     → semantic search: Voyage AI embeds the query → Pinecone finds similar
//            functions → Claude pinpoints the exact line → results show score,
//            plain English summary, and a green arrow pointing to the matched line
// ─────────────────────────────────────────────────────────────────────────────
...
// ── Render AI Results ─────────────────────────────────────────────────────────
// Renders semantic search results from the AI pipeline.
// Each result is a function with:
//   - relevance score (cosine similarity, sorted highest first)
//   - plain English summary (generated by Claude at index time)
//   - matched line (pinpointed by Claude at search time — green arrow → line N)
...
function renderAiResults(data) {
  ...
  results.forEach((r) => {
    ...
    // Use the LLM-pinpointed line if available, otherwise fall back to function start
    const openLine   = r.match_line || r.start_line;
    const lineLabel  = r.match_line
      ? `line ${r.match_line}`
      : `lines ${r.start_line}–${r.end_line}`;

    // Show the specific matched line if Claude found one, otherwise show code preview
    const matchedLineHtml = r.match_content
      ? `<div class="ai-match-line"><span class="ai-match-arrow">→</span><span class="ai-match-linenum">line ${r.match_line}</span><code class="ai-match-code">${escHtml(r.match_content)}</code></div>`
      : "";

    // Show LLM summary if available, otherwise show raw code preview
    const previewHtml = r.summary
      ? `<div class="ai-summary">${escHtml(r.summary)}</div>`
